...
{
    if (!ppszName) return E_POINTER;
    const std::wstring title = GetLocalizedString(StringId::HashSubmenu) + L": CRC-32";
...
{
    if (!ppszName) return E_POINTER;
    const std::wstring title = GetLocalizedString(StringId::HashSubmenu) + L": SHA-256";
    return SHStrDupW(title.c_str(), ppszName);
}
...
STDMETHODIMP PakkoRootCommand::EnumSubCommands(IEnumExplorerCommand** ppEnum) noexcept
{
    try
    {
        ...
        *ppEnum = nullptr;
        ...
        // fetched the first grandchild's title/state via COM (confirmed via live logging) but
        // never actually painted the flyout, leaving it visually empty. Flattening to direct
        // PakkoRootCommand children reuses the exact same single-level-nesting code path every
        // other leaf here already relies on. See DECISIONS.md's T-F128 follow-up entry.
