...
- [x] **Status:** done — implemented, tested, and on-device verified 2026-07-18 (agent-driven via
      the local `windows` MCP server, user-directed). See `DECISIONS.md`'s T-F117 entry for the
      full fix (and its T-F116 entry for the original repro).
- **Depends on:** none
...
**Acceptance criteria:**
- [x] `ZipArchiveService.ExtractAsync`/`TestAsync`'s per-item loop records a real `ArchiveError` (or
      ...
      (decided: `ArchiveError` — matches the existing "known-but-unsupported = skip,
      unrecognized/corrupted = error" split already in the same method)
- [x] `Archiver.Core.Tests` covers: empty file, plain garbage bytes, truncated real ZIP
- [x] `Archiver.CLI.Tests`' `Extract_SiWithEmptyStdin_SilentlyNoOpsPerPreExistingCoreBehavior`/
...
- [x] Checked whether `TarSandboxedService`'s extraction path has the same gap for a
      non-tar/non-recognized input, and fixed there too if so (for parity, not assumed) — checked,
      no gap: it has no upfront format short-circuit, so an unrecognized archive already fails
      loudly via tar.exe's `-tf` exit code → `IOException` → `ArchiveError`
- [x] `Archiver.App`'s operation-summary dialog still reads correctly for this case (a real error
      now appears where none did before — confirm it doesn't regress a currently-passing manual
      test path) — verified on-device 2026-07-18: launched the installed app via a real
      `pakko://extract` activation against a 76-byte garbage `.zip`, clicked "Розпакувати", and the
      operation-summary dialog correctly showed "Завершено з проблемами" / "Помилки (1)" /
      `garbage.zip` / "File is not a recognized archive format and cannot be extracted."

---

### T-F118 — ZIP vs. Tar-Family Extraction Smart-Foldering Asymmetry
- [x] **Status:** done — implemented, tested, and on-device verified 2026-07-18 (agent-driven via
      the local `windows` MCP server, user-directed). User-directed decision: unify tar-family to
      match ZIP's smart-foldering (not the reverse). See `DECISIONS.md`'s T-F118 entry (and T-F09
      "Implementation" entry for the original repro).
- **Depends on:** none
...
**Acceptance criteria:**
- [x] Decide the intended behavior — match tar-family to ZIP's smart-foldering, match ZIP to
      ...
      touching code (decided: match tar-family to ZIP, user-directed — see `DECISIONS.md`)
- [x] Whichever direction is chosen, both `ZipArchiveServiceTests` and
      `TarSandboxedServiceExtractionTests` (or equivalent) cover the multi-root-item, no-common-
      folder case explicitly — `TarSandboxedServiceExtractTests` gained three new tests plus one
      updated pre-existing one
- [x] `Archiver.Shell/Program.cs`'s `--extract-flat` doc comment corrected to match actual verified
      behavior, not the aspirational "no wrapper folder ever" claim
- [x] `Archiver.CLI.Tests`' existing test asserting the ZIP-wraps/tar-doesn't-wrap asymmetry
      updated to match whichever new unified behavior is chosen
      (`Extract_TarGzHappyPath_ExtractsFilesAndExitsZero`)
- [x] On-device verification — a real `Deploy.ps1` build + a genuine multi-root `.tar.gz` fixture
      (two root-level files, no common containing folder) extracted through the installed app via
      a real `pakko://extract` activation, confirmed on disk: output landed under a
      `multiroot\` subfolder (named after the archive) containing both files with correct content,
      matching `ZipArchiveServiceExtractTests.ExtractAsync_MultipleRootItems_
...
- [x] **Status:** done 2026-08-11. Found by the user personally, while extracting an archive they
  had created from a single file. `ZipArchiveService.ExtractWithSmartFolderingAsync` (and
  `TarSandboxedService`'s identically-shaped `ExtractSingleArchiveAsync`) already computed an
  `isSingleRootFile` flag but never actually used it to change where the file landed under
  `ExtractMode.SeparateFolders` (the App's default Extract button and Explorer's "Extract
  Here"/`--extract-here`) — a single-file archive like `photo.zip` extracted to
  `<Destination>\photo\photo.png` instead of `<Destination>\photo.png`, unlike a real archiver
  (NanaZip/7-Zip/WinRAR). `Archiver.CLI`'s `x` command was confirmed unaffected (always uses
  `ExtractMode.SingleFolder`, where the flag already worked correctly). Consulted `advisor` before
  implementing, given the fix touches two extraction engines at once. Fixed by threading a new
  explicit `unisolatedDestDir` parameter (the caller's real `options.DestinationFolder`, already
  available — no path-string derivation) into both engines' smart-foldering methods, gated
  precisely on `alreadyIsolated && isSingleRootFile && !isSelectedSubset` so `--extract-folder`'s
  own "always wrap regardless of structure" contract stays untouched. A second ripple caught before
  shipping: `ZipArchiveService`'s temp-staging path was derived from the final destination
  (`actualDest + "_tmp"`), which — once the final destination could become a *shared* folder
  several archives resolve to — would have been an unsafe colliding sibling path; re-anchored to
  the always-archive-specific `destDir` instead. Four pre-existing tests had accidentally used
  single-file fixtures to test something unrelated (per-archive subfolder creation, compound-
  extension stripping, `SeparateFolderName` override) and were widened to 2-file fixtures so their
  actual point survives the fix; a fifth genuinely was single-file and had its assertion corrected
  to the new (correct) path, doubling as a regression guard. New tests across three projects, each
  confirmed to actually fail against a temporary revert before being restored passing:
  `ZipArchiveServiceExtractTests` (the core fix, a negative case proving `--extract-folder` is
  unaffected, and a multi-archive-batch collision case), a real-`tar.exe`
  `TarSandboxedServiceCompressedFormatsTests` mirror, and a `pakko.exe` subprocess regression guard
  documenting the CLI was never affected. `dotnet test --filter "Category!=Slow&Category!=
  VeryLarge"` green repo-wide (824/824). `Deploy.ps1` build+sign+install (v1.4.10.1) and an
  agent-driven on-device check against the real installed, packaged app confirmed both entry points
  (App's Extract button via `pakko://extract`, and the real installed `Archiver.Shell.exe
  --extract-here`) land a single-file archive directly, byte-correct, no wrapper folder. See
  `docs/DECISIONS.md`'s T-F154 entry for the full account, including a genuinely new collision
  surface the fix exposes in Explorer's extraction (now resolved silently via auto-rename, no
  interactive prompt — reported to the user as a follow-up decision point, not yet actioned) and
  the answered-but-not-implemented conflict-dialog-shape question.
- **Not yet released:** `v1.4.10` was already tagged with its Store-signed MSIX built before this
  bug was found — it does not contain this fix. Lands in `v1.4.11` instead.
- **Reported by:** user, 2026-08-11 — "Знайшов дивну поведінку - при розпакуванні файлу який був
  зарахівований один він видобувається у папку зі своїм іменем і не у файл яким він був зархіований."
- **Depends on:** none

---

### T-F156 — SingleFolder ("flat") Mode Wrapped a Multi-Root Archive in a Subfolder, Contradicting Its Own Contract

- [x] **Status:** done 2026-08-11. Found by the
  user immediately after T-F154 shipped, testing extraction of an archive with several files and no
  common folder. `ExtractMode.SingleFolder`'s own doc comment says "all archives -> one flat
  folder", but a genuinely multi-root archive still landed inside an extra
  `<archive-base-name>\` subfolder — the same T-14 smart-foldering mechanism T-F118 (2026-07-18) had
  *deliberately, explicitly, on-device confirmed* made apply here too. This is a real conflict with
  a prior explicit user decision, not a plain bugfix — surfaced via `AskUserQuestion` quoting
  T-F118's own text rather than silently reversing it; user confirmed reversing it for SingleFolder
  mode specifically ("Так, скасувати для SingleFolder"), leaving SeparateFolders mode's own
  unconditional per-archive wrapping untouched. Root cause was a single ternary branch in
  `ZipArchiveService.ExtractWithSmartFolderingAsync`/`TarSandboxedService.ExtractSingleArchiveAsync`
  reachable only in exactly this case — removed outright rather than further gated, since no other
  caller could ever reach it. `Archiver.Shell`'s `--extract-flat` doc comment corrected back to "no
  wrapper folder ever created". 8 pre-existing tests across 4 projects (including T-14's own
  original v1.0-era `ExtractAsync_MultipleRootItems_CreatesSubfolderNamedAfterArchive` test, renamed
  to `..._ExtractsFlatWithoutSubfolder`) flipped to the new expectation, found by running the full
  suite rather than assumed exhaustive up front. New/flipped regression tests confirmed to actually
  fail against a scoped `git stash` of just the two service files (tests left in place) before being
  restored passing. Also closed a coverage gap the advisor flagged in T-F154's own review: a new
  test locks in that a single-root-file archive bypasses any caller-supplied `SeparateFolderName`
  too. `dotnet test --filter "Category!=Slow&Category!=VeryLarge"` green repo-wide (826/826).
  `Archiver.ShellExtension.Tests.exe` (C++, unaffected) reconfirmed 100/100. **Graduated to `[x]`
  2026-08-11** via `Deploy.ps1` build+sign+install (v1.4.10.2) plus an agent-driven on-device
  check against the real installed package: `Archiver.Shell.exe --extract-flat` against a real
  3-file no-common-folder ZIP landed all three files directly next to the archive with no wrapper
  subfolder; `pakko.exe x` (a fresh Debug build from this session) did the same into an explicit
  destination; `Archiver.Shell.exe --extract-here` (SeparateFolders mode) was separately confirmed
  UNCHANGED — it still wraps the same archive into its own per-archive subfolder, as designed.
  See `docs/DECISIONS.md`'s T-F156 entry for the full account, including the conflict with T-F118
  and the user's direct confirmation to reverse it.
- **Not yet released:** lands in `v1.4.11` alongside T-F154 — `v1.4.10` does not contain this fix.
- **Reported by:** user, 2026-08-11 — "додай тест багато файлів без папки. Він знову видобуваю у
...
failed") were hardcoded English, never localized

- [x] **Status:** done 2026-08-11 — implementation, tests, and on-device verification all
  complete.
- **Reported by:** the user, using T-F155's own conflict dialog against a real re-extraction of a
  personal archive into an already-populated destination (every entry genuinely conflicted, so
  choosing "Пропустити" + "apply to all" correctly extracted nothing) — the resulting summary
  dialog's header showed in English ("1 entry skipped:") despite running under `uk-UA`: "Чомусь
  повідомлення англійською хоча стоїть українська версія. Як ми могли пропустити таке?"
- **Root cause:** `ShellResultPresenter.BuildSkippedMessage` hand-rolled English noun
  pluralization (`"{0} entry/entries skipped:"`), and `Program.cs`'s `ShowErrorSummary`/
  `RunTestAsync` had three more hardcoded literals ("The operation failed.", "…and {0} more",
  "No errors detected in the archive(s)."). Predates and survived T-F128/T-F146/T-F155's own
  37-locale localization of `HashMessages`/`ScanMessages`/`ConflictMessages` — this specific
  dialog (T-F68, 2026-07-06) is older than all three and nobody had revisited it since. Per-item
  skip/error *reason* text stays English by design (`Archiver.Core` has zero localization
  dependency, matching the WinUI App's own T-F89 precedent) — only the Shell-owned header/label
  text was in scope.
- **Fix:** new `Archiver.Shell/Resources/ResultMessages.resx` (+ 36 locale files) and
  `ResultMessagesLocalizer` (same pattern as `HashResultLocalizer`/`ScanResultLocalizer`), 4 keys:
  `ResultSkippedHeader` ("Skipped ({0}):" — drops English's noun-pluralization branch entirely
  rather than translate it, matching the App's own T-F89 "Skipped (N)" convention), `ResultAndMoreLine`,
  `ResultNoErrorsDetected`, `ResultOperationFailed`. Two of the four values were reused verbatim
  from already-translated, already-shipped strings rather than re-translated — `ResultSkippedHeader`'s
  word from the App's `SkippedSectionHeader` (T-F89), `ResultAndMoreLine` from Shell's own
  `ScanMessages.ScanAndMoreLine` (T-F146) — only the two "no errors"/"operation failed" phrases
  needed genuinely fresh translation across 36 locales. `BuildSkippedMessage`/`ShowErrorSummary`/
  `RunTestAsync` now call `ResultMessagesLocalizer.Get(...)` instead of the hardcoded literals.
- **Testing (test-first):** updated `ShellResultPresenterTests.cs`'s three `BuildSkippedMessage_*`
  tests (explicit `en-US` culture pin — this dev machine's own OS UI culture is `uk-UA`, so an
  unpinned assertion first failed against the *correct* localized Ukrainian output, confirming the
  fix rather than a stale assertion). New `ResultMessagesLocalizerTests.cs` mirrors
  `HashResultLocalizerTests`' neutral/`uk-UA` smoke checks plus `ConflictDialogLocalizerTests`'
  all-37-locale × all-4-key `FormatException` loop. `dotnet test tests/Archiver.Shell.Tests` —
  447/447 green. `dotnet test --filter "Category!=Slow&Category!=VeryLarge"` green repo-wide.
- **On-device verification (2026-08-11, agent-driven via the local `windows` MCP server):**
  `Deploy.ps1` build+sign+install (v1.4.11.0). Built a minimal repro (destination folder with an
  existing `hello.txt`, a `test.zip` containing a conflicting `hello.txt`), launched the installed
  `Archiver.Shell.exe --extract-flat test.zip` directly, checked "Застосувати до всіх решти
  конфліктів" and clicked "Пропустити" on the real native `ShellConflictDialog` (T-F155) — this
  time `ui_click` worked directly on both the checkbox and the button, unlike T-F161's on-device
  pass, which needed keyboard-navigation fallback for the same dialog class. Confirmed the
  resulting summary `MessageBoxW` reads "Пропущено (1):\ntest.zip: No entries were extracted from
  this archive — every entry was skipped." — header correctly localized to Ukrainian; per-item
  reason text stays English by design.
- **Not fixed, by design:** the user separately asked whether showing a warning-icon summary
  dialog at all is "best practice" when the user explicitly chose Skip themselves (contrasting
  NanaZip's silence in this scenario). Left unchanged — this same code path also fires for skips
  the user never explicitly agreed to (ADS/reserved-name/zip-bomb/unsupported-format rejections),
  and `SkippedFile` carries no marker distinguishing those from a T-F155 self-chosen Skip, so
