...
## T-F108 — Archive Browser Silently Extracts to Desktop, Ignoring the Archive's Real Location

**Found while:** scoping T-F98 (see below) — the user reported that double-clicking a
non-previewable file (`.mp4`) in the Archive Browser "does nothing." Automating a repro via the
`windows` MCP server hit a wall (WinUI's `DoubleTapped` gesture wasn't reliably synthesizable by
the available mouse tools after several attempts across two different Explorer/PickerHost
interaction paths), so the user reproduced it manually instead and reported the real symptom:
the file **does** extract — silently, into whatever folder `DestinationPath` held, which was
`Desktop`, surprising since the archive itself lived elsewhere entirely.

**Root cause:** `MainViewModel._destinationPath` field-initializes to `Desktop` and is only ever
updated by `UpdateDefaultDestination()`, wired solely to `FileItems.CollectionChanged` (the
pending-list collection). `EnterBrowseModeAsync` — the entry point T-F100's file-activation
routing ("Open with → Pakko") and T-F107's real-filesystem archive drill-in both call — never
touches `FileItems`, so on those two paths `DestinationPath` stays stuck at `Desktop` regardless
of where the opened archive actually lives. T-F05's own original entry point (double-clicking an
archive already in the pending list) never surfaced this, since `FileItems` already held that one
item and `UpdateDefaultDestination()` had already set the correct folder when it was added.

**Fix:** `EnterBrowseModeAsync` now sets `DestinationPath = Path.GetDirectoryName(archivePath) ??
DestinationPath` when `FileItems.Count == 0` — i.e. only when no pending-list-derived default
exists yet, so a destination a user picked manually via the pending-list flow is never
overwritten. Deliberately scoped to `EnterBrowseModeAsync` itself, not the shared
`ListArchiveWithProgressAsync` listing helper both it and T-F98's `NavigateIntoNestedArchiveAsync`
...
top of each level's own 1000:1 ratio check (T-F90/T-F94) — user chose 4. Implemented as
`Archiver.App.Core.NestedArchivePolicy.MaxDepth`/`ExceedsMaxDepth`, not `Archiver.Core` — this is
a browse-session/UX resource bound the App layer's recursive orchestration enforces, not a
per-extraction security invariant; that backstop is T-F49/T-F90/T-F94, unchanged and already
reused per-level (see below).

**Per-level security — reuses the existing single-entry extraction pipeline unmodified, adds
zero new Core code.** Confirmed by reading `TarSandboxedService.ExtractSingleArchiveAsync` and
`ZipArchiveService.ExtractWithSmartFolderingAsync`: passing `ExtractOptions.SelectedEntryPaths`
with exactly one path still runs T-F49's whole-archive pre-scan and T-F90/T-F94's whole-archive
compression-ratio + disk-space check unconditionally, scoped to whatever archive is being
extracted at that call. `NavigateIntoNestedArchiveAsync` (`MainViewModel.cs`) is, at the Core
layer, nothing more than calling `IExtractionRouter.ExtractAsync` again against whatever was just
extracted — exactly the same call T-F97's `PreviewBrowserEntryAsync` already makes for a single
preview. `NestedArchiveDrillDownSecurityTests.cs`
(`ExtractAsync_NestedBombArchive_RejectedIndependentlyAtSecondLevel`,
`Archiver.Core.IntegrationTests`) proves this two-level composition directly: an outer (clean)
tar containing a real bomb-shaped nested `tar.gz` extracts the nested archive out fine at level 1,
then the same bomb check independently skips it at level 2 with no confirm callback wired —
proving nesting doesn't bypass or weaken either mechanism.

**Format detection before extraction — extension only, confirmed for real after.**
`ArchiveFormatDetector.Detect` is magic-byte-only and requires a real file on disk, so it cannot
classify an in-archive entry before extraction. New `ArchiveFormatDetector.IsRecognizedArchiveExtension`
(pure extension lookup, no disk I/O) decides drill-down *candidacy* for the double-click dispatch
in `MainWindow.xaml.cs`; the real `Detect()` call still runs against the extracted temp file
before entering browse mode, rejecting cleanly (not crashing) if an entry merely named like an
archive isn't really one. This method absorbed `MainViewModel`'s previously-separate
`_extractableTypes` `HashSet` (used by `IsExtractOnlySelection`) — one owner for "is this a
recognized archive extension" instead of two independently-maintained copies.

**Temp lifecycle — a new cache, not a rename of `PreviewCache`, because the cleanup timing
requirement genuinely differs.** `PreviewCache` (T-F97) must wait for window close, since an
external OS handler may still have the previewed file open. A nested-archive scope has no such
constraint — nothing outside Pakko ever holds a handle into it once the user navigates back out —
so `NestedArchiveCache` adds `DeleteScope(scopeDir)` for immediate per-level cleanup on `NavigateUp`
popping a level, bounding disk usage across a long drill-down/back-out session, with `DeleteAll()`
kept as a crash/abandoned-session safety net (called from `MainWindow.xaml.cs`'s `Closed` handler,
same as `PreviewCache.DeleteAll()`).

**Browse-stack model, not a redesign of `CurrentFolderPath`/`BrowsedArchivePath`.** A private
`Stack<NestedBrowseLevel>` in `MainViewModel` holds each ancestor level's restorable state
(`ArchivePath`, `CurrentFolderPath`, display name, archive index, breadcrumb ancestry, scope dir)
— `BrowsedArchivePath`/`CurrentFolderPath`/the folder index keep meaning exactly what they always
have (the *current*, possibly-nested archive), so every other browser command (Extract
Selected/All, `NavigateIntoFolder`) needed zero changes. `NavigateUp` checks the stack before
falling through to T-F107's real-filesystem climb — only once every nested level is popped does
the outermost archive's own root reach that climb, unchanged from T-F107.

**Breadcrumb: continuous display, but ancestor segments aren't directly clickable — a deliberate,
recorded scope cut, not an oversight.** `RebuildBreadcrumb` prepends a `_nestedBreadcrumbAncestry`
list (each ancestor level's own root-name + folder-path contribution) so the breadcrumb reads as
one continuous path across nesting (`outer.zip > sub > inner.rar > docs`), and
`_currentLevelDisplayName` overrides the usual `Path.GetFileName(BrowsedArchivePath)` for a nested
level (whose `BrowsedArchivePath` is an ugly `NestedArchiveCache` temp file, not the real entry
name drilled into). Clicking an ancestor breadcrumb segment directly (jumping back several
nesting levels in one click) is **not wired** — `NavigateToBreadcrumbSegment`'s Archive branch
computes `localIndex = index - _nestedBreadcrumbAncestry.Count` and simply returns (no-op) when
`localIndex < 0`. Fully supporting this would need tracking, per ancestor frame, exactly which
global breadcrumb indices it owns so a click could pop the right number of levels and land on the
right folder within that level — real complexity for a case `NavigateUp` already reaches correctly
one level at a time, and the approved plan never committed to it. Recorded here so it isn't
...
## T-F109 — Archive Browser Preview Allowlist: Widened to Video/Audio, Verified Against Real 7-Zip/NanaZip Behavior

**Trigger:** after T-F108 fixed the destination bug, the user clarified the original mp4 report
wasn't just about *where* it landed — mp4 double-click should behave like T-F97's existing
text/image preview (silent extract to the shared temp cache, launch with the OS default player,
cache removed on window close), not run the full Extract-to-destination flow at all.
`PreviewPolicy.IsPreviewable` had never included video. When asked whether to expand the
allowlist further, the user asked how other archivers handle this — a legitimate question this
project's own convention answers by checking real shipped source, not memory or assumption.

**Research (per `CLAUDE.md`'s pre-implementation-research rule):** fetched NanaZip's real
`NanaZip.UI.Modern/SevenZip/CPP/7zip/UI/FileManager/PanelItemOpen.cpp` via
`raw.githubusercontent.com`. Its double-click handler (`CPanel::OpenItemInArchive`) has **no
file-type allowlist whatsoever**: it always extracts the entry to a `CTempDir` and unconditionally
calls `ShellExecuteEx` on the result via `StartApplication`, regardless of extension — including
`.exe`, which gets special-cased to extract *every* sibling file first
(`FindExt(kExeExtensions, name)` triggers extracting the whole archive, not just the one entry, so
a portable app's DLL dependencies resolve next to it). The only gate present anywhere in this path
is `IsVirus_Message` — a check for the Unicode right-to-left-override (`0x202E`) filename-spoofing
trick, not a type restriction; it fires regardless of what's about to be opened, previewable or
not. **Conclusion: Pakko's safe-type allowlist (T-F97) is not an industry convention it's
following — it's a deliberate, stricter-than-reference security posture, consistent with
`SECURITY.md`'s stated minimal-attack-surface positioning for a government/defense audience.**
Recorded so a future session doesn't assume the allowlist should be relaxed "to match how real
archivers do it" — real archivers don't do it this way; Pakko's choice to differ is intentional.

**Decision — widen the allowlist, but keep it a genuine allowlist, not drop it:** user chose to
extend `PreviewPolicy` with common audio containers (`.mp3`/`.wav`/`.flac`/`.ogg`/`.m4a`/`.aac`) in
addition to the video containers just added, explicitly declining PDF — despite PDF looking like
an obvious "safe document" candidate, some readers execute embedded JavaScript from a PDF, unlike
every other type on this list, so it stays outside the allowlist deliberately (a `[InlineData]`
case in `PreviewPolicyTests` locks this in as a test, not just a comment, so a future PR adding PDF
"for convenience" fails a real assertion instead of silently slipping through).

**Decision — non-allowlisted types get a warning + a different destination, not a silent full
Extract:** previously, a non-previewable double-click silently ran the existing full Extract flow
straight to whatever the user's Destination field held (T-F108 had just fixed *that* field's
correctness, but the flow itself stayed a silent one-click full extraction of arbitrary archive
content). User decided this needed real friction: a confirm dialog
(`IDialogService.ShowConfirmAsync` — the existing generic Yes/No dialog, no new dialog type
needed) explaining the file can't be safely opened directly, and on confirmation the entry
extracts into a **subfolder next to the archive on disk** (`Path.Combine(archiveDir,
ArchiveNaming.GetBaseName(archivePath))`) rather than the Destination field — that field is for
deliberate bulk Extract Selected/All operations the user configured, not a one-off security-gated
extraction triggered by an accidental double-click. `MainViewModel.RunExtractAsync` gained an
optional `destinationOverride` parameter for this rather than temporarily mutating
`DestinationPath` and restoring it, avoiding any re-entrancy/race risk with the field's normal
two-way binding. `ExtractSingleBrowserEntryAsync` was renamed to
`ExtractSingleBrowserEntryWithWarningAsync` and had the confirm+destination logic added directly —
not kept as a separate unused method, since it had exactly one caller.
...
## T-F110 - Archive Browser: Per-Row Preview/Extract-Only Icon, Not a Dialog Replacement

**Trigger:** the user found T-F109's confirm dialog (for a non-previewable double-click) potentially naggy, and proposed a passive visual signal instead - either greying out non-previewable rows, or a mark in the entry table's first column (an eye / crossed-out eye was their own suggestion). Explicitly asked for both a design consult (`frontend-design` skill) and a separate advisor pass before implementing.

**Design decision - extend the existing column-0 icon, don't add a column or grey out rows.** `ArchiveEntryViewModel.Icon` already showed a generic file glyph for every non-folder row (no per-type distinction at all before this). Greying out rows was rejected: every row stays fully double-clickable regardless of preview-vs-extract-only status, and greyed text is an established convention for *disabled/non-interactive* controls elsewhere in this codebase (e.g. IsNotBusy-gated option controls) - reusing it here would misleadingly suggest these rows can't be clicked. A new dedicated column was also rejected: this project already has a documented restraint precedent (T-F92 - per-command context-menu icons were implemented, then reverted once actually rendered and judged clutter) and adding a column carries real risk given this table's own history of column-alignment bugs (T-F106's Grid MinHeight issue, the earlier Archive Options label-column bug). Changing what the *existing* icon means costs nothing extra, so it was the natural choice. Glyphs: Segoe MDL2 Assets' `View` (U+E890, an eye) for a `PreviewPolicy`-allowlisted file, `Hide` (U+ED1A, a crossed-out eye) for anything else - same font family this XAML already uses everywhere else (the Up-arrow buttons, the empty-state drop-zone hint), verified against Microsoft's published Segoe Fluent Icons glyph list (same codepoints carried forward from Segoe MDL2 Assets for icon-font backward compatibility) rather than guessed from memory.
...
## T-F09 — Distribution: Standalone Downloadable CLI, `tar.exe` Not Bundled

**Context (2026-07-18):** while T-F09 was still `future`/unimplemented, the user asked whether
`Archiver.CLI` could ship as its own separate, self-sufficient downloadable product — its own
per-architecture folder, hashes for verification, and a bundled copy of Windows' `tar.exe`
alongside it — while still reusing `Archiver.Core` under the hood ("прозора архітектура... щоб
цей бінарник реюзав наше ядро").

**Chosen:** the "reuse Core" half was already exactly what `CLI.md`'s Architecture section
describes — `Archiver.CLI` is a third thin frontend consuming `Archiver.Core` in-process, same
pattern as `Archiver.App`/`Archiver.Shell`, not a new engine. The new part — a standalone,
independently downloadable artifact per architecture with published hashes — is accepted as a
packaging/release-engineering addition, published self-contained via GitHub Releases (the
existing v1.1+ channel) with a `SHA256SUMS` file per build. Added to `CLI.md`'s new
"Distribution" section and to T-F09's acceptance criteria in `TASKS.md`.

**Rejected: bundling a redistributed copy of `tar.exe`.** `Archiver.CLI` only runs on Windows
(tar/RAR/7z support depends on `Archiver.Core`'s Windows-only `TarSandboxedService`), and the
OS's own `C:\Windows\System32\tar.exe` is always present on any Windows version this project
targets (Win10 1803+/Win11) — this project's existing hard constraint already mandates calling
it by that absolute path, never via PATH, and never a bundled substitute (see `CLAUDE.md`'s hard
constraints). Shipping a second copy inside the CLI's download would add a new supply-chain
surface (a binary to hash-pin and license-audit on top of the vendored `7za.exe` T-F114 already
introduced, test-only, for perf comparisons) for zero functional gain, since the OS-provided
binary is guaranteed available anyway. User confirmed this trade-off explicitly rather than the
agent assuming it.

**Not yet decided / left for T-F09 implementation time:** exact GitHub Releases asset naming,
whether `SHA256SUMS` is generated by a new small script under `scripts/` (per this project's
"prefer simple explicit scripts over build-pipeline hooks" rule) or as a manual release step, and
whether ARM64 CLI builds are cut on the same cadence as x64. None of these affect the two
decisions above and can be resolved when T-F09 actually starts.

---

## T-F09 — Implementation: Exit Codes, -y/-ao Wiring, -mx Bucketing, -t Spellings

**Context (2026-07-18):** T-F09 (`Archiver.CLI`) moved from `future` to implemented this session,
following the "Distribution" decision recorded above. This entry records the concrete decisions
made during implementation that the Distribution entry explicitly deferred, plus one real finding
from subprocess testing.

**Exit codes** (kept 7z-familiar): `0` success/nothing skipped, `1` success-with-warnings
(`ArchiveResult.SkippedFiles` non-empty — e.g. a conflict/bomb declined without `-y`, or `t` hit a
tar-family archive), `2` operation failed (`ArchiveResult.Success == false`, listing failed, or
real Test corruption), `7` command-line error (all three of `CLI.md`'s three-way-rule categories —
distinguished by stderr text, not exit code, since that's what `TASKS.md`'s acceptance criteria
actually asks to be tested).

**`-y`/`-ao` wiring:** `ArchiveOptions`/`ExtractOptions` already default to `Skip`/auto-decline
when `ResolveConflictAsync`/`ConfirmCompressionBombExtraction` are null — so the CLI needs to do
nothing special in `-y`'s absence; the null-callback defaults are already the safe, non-interactive
behavior a CLI needs. `-y` only overrides them (`ConfirmCompressionBombExtraction` → always-true,
`OnConflict` → `Overwrite`), and an explicit `-ao{a|s|u}` always wins over `-y` when both are given
(`command.OverwriteMode ?? (AssumeYes ? Overwrite : Skip)`). `ResolveConflictAsync` stays null
always — the CLI never sets `OnConflict = Ask`, so per-conflict prompting never applies here.

**`-mx` bucketing** (`CliCompressionLevelMapper`): `0`→`NoCompression`, `1-2`→`Fastest`,
`3-6`→`Optimal` (7z's own default `-mx5` lands here, matching `ArchiveOptions`' own default),
`7-9`→`SmallestSize`. Note: the real `System.IO.Compression.CompressionLevel` enum member is
`SmallestSize`, not `SmallestFiles` — `CLI.md`'s original switch-table prose said `SmallestFiles`
(never compiled against the real enum until this session); corrected in both `CLI.md` and the code.

**`-t<type>` spellings for `a`:** `-tzip` (default), `-ttar`, `-ttar.gz`, `-ttar.bz2`, `-ttar.xz`,
`-ttar.zst`, `-ttar.lzma` — 1:1 with `ArchiveContainerFormat`. `-t7z`/`-trar` are recognized (not a
parse-time typo) but rejected with a named reason — Pakko can only *create* ZIP/tar-family.

**`CLI.md` staleness correction (Phase 0 of implementation, before any code was written):** the
`a` (Archive) and `l` (List) rows, and the `-t{type}` switch row, still described a pre-T-F105/
pre-T-F05 world (`CLI.md` was last edited 2026-07-13, before both of those shipped). Verified
against real current source (`IArchiveCreationRouter.cs`, `IArchiveListingRouter.cs`) rather than
trusting the doc, then corrected `CLI.md` itself — `a` now creates all 7 formats, `l` is fully
supported. This is exactly the kind of doc drift `CLAUDE.md`'s Documentation Map exists to catch;
flagging it here since the correction happened mid-implementation, not as a separate audit pass.

**Real finding from subprocess testing (not a bug — a documented existing behavior CLI inherits
unmodified):** `x` against a real ZIP with two root-level files and no common containing folder
lands them inside a `<destDir>/<archive-base-name>/` wrapper folder, not directly in `destDir` —
`ZipArchiveService.ExtractAsync`'s existing "smart foldering" (T-14) applies even under
`ExtractMode.SingleFolder`, since `alreadyIsolated` there is `false` (it's only `true` for
`SeparateFolders`, which pre-computes its own outer subfolder). `Archiver.Shell`'s own
`--extract-flat` (T-F115) command uses the identical `SingleFolder` setup and inherits the exact
same behavior, despite that command's doc comment describing it as "no wrapper folder ever
created" — that comment is accurate for the common single-root-folder case but not this multi-
root-item edge case. `ITarService.ExtractAsync` has no equivalent smart-foldering at all — the
same scenario against a `.tar.gz` lands files directly in `destDir`, no wrapper. Confirmed via a
real subprocess test exercising both paths (`CliSubprocessTests.cs`). Not fixed here — `Archiver.
CLI` is a thin frontend reusing Core's extraction engine unmodified, per its own architecture
principle (`CLI.md`'s "Architecture" section); this is a pre-existing Core behavior difference
between the ZIP and tar-family extraction paths, out of scope for a CLI-only task. Worth a future
ticket if the inconsistency between the two engines is ever worth reconciling.

## T-F116 — Archiver.CLI stdin/stdout streaming (`-si`/`-so`)

**Context (2026-07-18):** while confirming T-F09 was done, the user asked whether the CLI supports
piping raw archive bytes through stdin/stdout (7z's `-si`/`-so`), scoped this as its own task with
a testing requirement, then asked for the plan to be redone through `advisor` before implementing.
The advisor review surfaced a gap the first draft missed entirely: the user's literal ask was
"works in PowerShell pipelines," and PowerShell's own pipeline semantics are not guaranteed to
preserve raw bytes — a test suite built only on `Process.Start`/`RedirectStandardInput/Output`
would pass green without proving that.

**Empirical PowerShell/cmd binary-pipe findings (built two throwaway raw-byte passthrough console
apps, pushed a 550-byte file with nulls/`0x1A`/CR-LF/high bytes through real shells, compared
`SequenceEqual` on each end):**

| Pattern | Shell | Result |
|---|---|---|
| `<` input redirection | PS 5.1 and 7.6 | Doesn't exist — reserved-operator parse error, both versions |
| `native \| native \| native > file` | PowerShell 7.6 | Byte-perfect (550/550) |
| `native \| native \| native > file` | Windows PowerShell 5.1 | **Corrupted** (1106/550 — always mediates native-to-native pipes as text, never a raw OS pipe) |
| `Get-Content -Encoding Byte \| native` | PowerShell 5.1 | Also corrupted, worse (4402/550) — not a viable workaround |
| `cmd /c "native \| native > file"` | Both, including invoked from inside PS 5.1 | Byte-perfect (550/550) everywhere |

Conclusion: this is a real, honest caveat about the *recipient* shell, not a gap in `Archiver.CLI`
itself — the CLI is correct at the OS-pipe boundary either way. Documented in `CLI.md` and
`CliHelpText.Text` (not just here) since a user needs this before writing a broken script, not as
an implementation footnote. `tests/Archiver.CLI.Tests/Subprocess/CliSubprocessTests.cs`'s
`CmdPipe_ArchiveSoToExtractSi_RoundTripsBytesExactly` exercises the actual `cmd /c "..."` recipe
end-to-end (launches `cmd.exe` itself, not `Archiver.CLI.exe` directly) so the test matches the
documented recipe, not just .NET's own `Process` plumbing.

**Design, unchanged from the pre-advisor draft:** implement `-si`/`-so` entirely inside
`Archiver.CLI/CliStreamStaging.cs` as buffer-then-proceed / proceed-then-stream through a private
`%TEMP%` folder, zero `Archiver.Core` changes. `ZipArchive` needs a seekable stream to read its
central directory (a raw stdin pipe can't be opened as a read archive directly); T-F49's
whole-archive pre-scan needs a real file to scan before extraction runs; `SandboxedProcessLauncher`
has no stdin-redirection plumbing; `ArchiveFormatDetector.Detect` sniffs magic bytes, not the file
extension, so a staged temp file with a meaningless name still routes correctly. A true zero-copy
design was considered and rejected — it would ripple into `IArchiveService`/`ITarService`'s public
contract (shared with App/Shell) and `SandboxedProcessLauncher`'s Win32 plumbing, to bypass T-F49's
pre-scan security model, for a payoff (avoiding transient disk use) not justified by that risk.

**`CliStreamStaging.StreamSingleFileAsync` takes the destination `Stream` as a parameter**
(`StreamSingleFileToStdoutAsync` is a thin wrapper defaulting to `Console.OpenStandardOutput()`) —
deliberately, so the broken-downstream-pipe path is unit-testable with a fake stream that throws
`IOException` on write (`CliStreamStagingTests.StreamSingleFileAsync_DestinationThrowsIOException_
ReturnsCleanErrorNoThrow`). An earlier attempt tested this by starting a real
`Archiver.CLI.exe` subprocess, reading a few stdout bytes, then closing the parent's read handle
to force a broken pipe — abandoned as unreliable: whether the child's write actually lands *after*
the close (vs. the whole payload already having cleared into OS pipe buffering before the close
call executes) is a genuine race, confirmed empirically (the test passed with exit 0 — no broken
pipe detected — even against a 5 MB poorly-compressible fixture sized well past the internal
81920-byte copy buffer). The unit-test-with-a-throwing-stream approach exercises the identical
catch block deterministically instead.

**Real finding, not a T-F116 regression — a pre-existing `ZipArchiveService` behavior inherited
unmodified:** feeding `-si` an empty or garbage (non-archive) stdin stream does **not** produce
exit code 2 as the original plan assumed without checking — it silently exits `0` with nothing
extracted and no error/skipped-file record. Root cause: `ZipArchiveService.ExtractAsync`'s
per-item gate (`if (!IsZipFile(archivePath)) { ... continue; }`) only records a `SkippedFile` when
`GetKnownArchiveReason` recognizes the bytes as a *different* known archive format (GZip/BZip2/
RAR/etc.); for bytes matching no known magic number at all, `reason` is `null` and the loop just
`continue`s with nothing recorded. Confirmed this is unrelated to stdin staging specifically by
reproducing the identical exit-0 result against a real on-disk garbage `.zip` via plain `x` (no
`-si` involved). Not fixed here — fixing it would mean changing `ZipArchiveService`'s per-item
loop, a `Archiver.Core` change T-F116 deliberately stays out of; `CliSubprocessTests.cs`'s
`Extract_SiWithEmptyStdin_SilentlyNoOpsPerPreExistingCoreBehavior`/`Extract_SiWithGarbageBytes_
SilentlyNoOpsPerPreExistingCoreBehavior` tests assert the real behavior rather than the assumed
one. Worth a future ticket on its own — a single unrecognized archive path silently doing nothing
is arguably worse UX than a named error, independent of `-si`.

## T-F116 — Follow-up: CLI Binary Renamed to `pakko.exe`; PATH/App-Execution-Alias Research

**Context (2026-07-18):** user asked for a short, recognizable binary name (suggested
`pakkos`/`pakkoc`) and whether the CLI gets added to `PATH` automatically, and asked for research
into how other open-source Windows console tools (ripgrep, fd, bat) handle this, before renaming.

**Research findings:** ripgrep/fd/bat's own zip distributions do **not** self-install to `PATH` —
the documented instruction is always "extract, then add the folder to `PATH` yourself," or use a
package manager (winget/scoop/chocolatey) which manages `PATH`/shims as part of install. No
zip-distributed tool auto-mutates `PATH` on first run. Confirmed `Archiver.CLI` matches this
existing norm rather than being an outlier.

**Real pitfall found, specific to this project:** Windows auto-adds
`%LOCALAPPDATA%\Microsoft\WindowsApps` to every user's `PATH`, positioned such that a registered
MSIX `AppExecutionAlias` there resolves ahead of most user-added `PATH` entries — the exact
mechanism behind the well-known "Microsoft Store `python.exe` stub silently shadows a real Python
install" problem (`where`/`Get-Command -All` is the standard way to diagnose it). Since Pakko
itself is an MSIX package, this is a live consideration, not a generic one: if the GUI is ever
given a terminal `AppExecutionAlias`, it must not claim the same name as the CLI binary, or
resolution order — not either binary's own code — decides which one a bare invocation launches.
Checked `Package.appxmanifest`: no `AppExecutionAlias` is registered today, so there is no current
collision, only a future-proofing constraint. Documented in `CLI.md`'s Distribution section so
it isn't lost.

**Decision:** renamed the built exe from `Archiver.CLI.exe` to `pakko.exe` via
`Archiver.CLI.csproj`'s `<AssemblyName>` (project folder/namespace/`.sln` entry all stay
`Archiver.CLI` — only the build output name changed, a one-line diff). Chosen over
`pakko-cli.exe`/other suggestions because it's already what every stderr message
(`pakko: <error>`) and `CliHelpText.Text`'s own `USAGE:` line say — the executable's real name now
finally matches what `--help` already told users to type, closing a latent inconsistency that
existed since T-F09 shipped. Updated: `CliProcessRunner.Resolve()`'s expected filename,
`Publish-Cli.ps1`'s zip naming (`pakko-<rid>.zip`, was `Archiver.CLI-<rid>.zip`), and every living
reference doc (`ARCHITECTURE.md`, `TESTING.md`, `scripts/README.md`, `README.md`, `CLI.md`).
Historical `DECISIONS.md` entries above that say `Archiver.CLI.exe` were deliberately left
unchanged — accurate at the time they were written, and this file is a log, not a snapshot to keep
current.

**PATH registration:** deliberately not implemented this round — no installer exists for the CLI
today (only the GUI is MSIX-packaged), and building one is a separate, larger scope than a rename.
Documented as a possible future follow-up (`winget`/`scoop` manifest, which would be the natural
"real install" path) in `CLI.md`, not turned into a numbered task yet since it wasn't requested as
one.

**Verified after rename (real exe, not just `dotnet test`):** clean rebuild produces `pakko.exe`;
full `a`/`l`/`t`/`x`/`i` command cycle against a real ZIP; all three three-way-rule error cases;
and — the case that actually exercises the renamed binary through a real OS pipe — `cmd /c "pakko
a -so single.zip file1.txt | pakko x -si -o pipedest > log"`, which correctly round-tripped the
file's content. `dotnet test --filter "Category!=Slow&Category!=VeryLarge"` stays green repo-wide
...
### T-F161 — `Directory.Move` fast-path silently defeated by a transiently-locked file, found via
real use of T-F155

Immediately after T-F155 shipped, the user hit a real crash using it: extracting a real
~4-file/1-folder archive a second time into an already-populated destination, choosing
Rename+"apply to all" on the conflict prompt, produced `"Cannot extract archive: Access to the
path '...\SICHER! B2 CD_tmp' is denied."` — the loose files landed correctly, but the folder never
appeared, and the whole operation reported failure. The user explicitly asked for a reproducing
test to be written **before** any fix was attempted, and offered their own hypothesis
("something prematurely deletes the temp folder") as a guess, not a diagnosis.

**Investigation, in order:** read `ExtractWithSmartFolderingAsync`'s full commit-phase logic
(`ZipArchiveService.cs`) end to end first — the fast path is a single `Directory.Move(tempDest,
actualDest)` when `actualDest` doesn't already exist, else a per-file merge-and-delete. Nothing in
that code deletes `tempDest` "early" as the user guessed. The exception text's shape (a bare
`"..._tmp"` path with no nested file name, wrapped in the `IOException` catch branch's own
`"Cannot extract archive:"` prefix) pointed at the fast-path `Directory.Move` call itself throwing
on the whole tree, not a specific file operation. Confirmed via an isolated, fixture-only platform
probe (no production code touched yet, matching this project's own pre-implementation-research
discipline): opened a file several directories deep inside a source tree with `FileShare.None`,
then called `Directory.Move` on the tree's top-level ancestor. Result: a plain `IOException`,
`"Access to the path '<top-level-source>' is denied."` — confirming Windows' `MoveFileEx` fails the
entire rename the moment *any* descendant file is locked by another process, and the exception
names only the directory passed to `Directory.Move`, never the actual locked file several levels
down. A real Explorer extraction can easily hit this transiently — a cloud-sync filter driver or
real-time antivirus briefly reopening a just-written file is exactly the kind of race this project
has already documented elsewhere (`Deploy.ps1`'s own T-F96 `AppPackages` wedge note describes the
identical class of transient external lock).

A second bug was found by continuing to read the same method rather than stopping at the first
fix: the entry-extraction loop's `try/catch` only cleaned up `tempDest` on
`OperationCanceledException`. Any *other* failure mid-loop (a T-F37/T-38-style rejection, a locked
source entry, disk-full) left the `"_tmp"` staging folder on disk forever — independently
plausible as how a stray `_tmp` folder could end up sitting on a real destination to begin with,
compounding the fast-path bug on a later re-run.

**Fix:** the commit logic was pulled out into `internal static
ZipArchiveService.CommitTempDestToActualDest(tempDest, actualDest, moveOverride = null)` so it
could be exercised directly by a test (via the existing `InternalsVisibleTo` grant to
`Archiver.Core.Tests`) without needing a real external file lock. The fast path now catches
`IOException` from the move and falls through to the per-file merge instead of propagating —
tolerating one still-locked file rather than losing the archive's entire folder to it — and the
merge path's trailing `Directory.Delete(tempDest, ...)` was made best-effort, since by that point
every real file has already been relocated and a lock on the now-empty staging directory structure
must not fail an otherwise-successful extraction. The entry loop's `catch (OperationCanceledException)`
was widened to `catch (Exception)` so every failure path — not just cancellation — cleans up
`tempDest` before rethrowing.

**Testing, test-first as the user asked:** `DirectoryMove_FileLockedDeepInsideSourceTree_
ThrowsIOExceptionNamingOnlyTopLevelPath` (the platform probe itself, committed as a permanent fact
lock-in, not a throwaway) was written and run to confirm the root-cause hypothesis *before* any
production code changed. `ExtractAsync_EntryPathTraversal_DoesNotLeaveOrphanedTempFolder` was
written next and confirmed failing against the unmodified code (a rejected traversal entry left
`out_tmp` on disk) before the `catch` widening was applied. Two direct tests against the new
`CommitTempDestToActualDest` (fast-path-success and fallback-on-`IOException`) round out the
file — the fallback test was confirmed failing (an unhandled `IOException` propagating out of the
test's own injected failing move) against a temporary revert of the `try/catch` before being
restored passing. All four were then re-verified failing against a scoped revert of just the
relevant hunk and restored — this project's own mutation-check discipline, applied exactly as
CLAUDE.md's "regression test... temporarily revert the fix" rule requires.
`Archiver.Core.Tests` grew 501 → 505; `dotnet test --filter "Category!=Slow&Category!=VeryLarge"`
green repo-wide. `TarSandboxedService`'s own move phase (`TryMoveSingleEntryAsync`) was checked and
found not to share this shape — it moves one file at a time (`File.Move`), never a whole subtree
via `Directory.Move`, so no parallel fix was needed there.

**On-device verification (2026-08-11):** `Deploy.ps1` build+sign+install produced v1.4.10.7. Per
this project's documented pattern for verifying `IExplorerCommand`-triggered behavior without
fighting Explorer's own context-menu automation, `Archiver.Shell.exe` was launched directly with
the exact args the real menu commands construct. `--extract-flat` against a 4-file/1-folder
archive, run twice into the same already-populated destination folder, reached the exact merge
branch this fix touches (`SingleFolder` mode's `actualDest` already exists on the second run).
The real `ShellConflictDialog` appeared; "Застосувати до всіх решти конфліктів" was checked and
"Перейменувати" clicked via keyboard navigation (native `TaskDialogIndirect` buttons still resist
`ui_click`/`mouse_control`, the same limitation already documented in T-F155's own entry above).
Result: both loose files renamed to `(1)` variants, both nested files under the pre-existing
