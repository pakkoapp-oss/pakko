...
/// <summary>
/// Marks a test as requiring a tar.exe build that supports the named format ("rar", "7z", "zstd",
/// "xz", "lzma", "bz2"). Skips if tar.exe is absent, or if DetectCapabilitiesAsync reports the
/// format unsupported (e.g. RAR5/7z require Windows 11 23H2+). Deliberately duplicated from
/// Archiver.Core.IntegrationTests' SkipIfFormatUnsupportedAttribute rather than cross-referencing
/// that test project, same reasoning as RequiresTarExeAttribute.
/// </summary>
public sealed class RequiresTarCapabilityAttribute : FactAttribute
{
    public RequiresTarCapabilityAttribute(string format)
    {
